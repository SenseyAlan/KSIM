...
У дослідженні аналізується залежність середньої чисельності популяції кроликів протягом 500 тактів від початкової чисельності, заданої на початку симуляції. Експерименти виконуються для початкових значень від 150 до 500 із кроком 50, що становить загалом 8 симуляцій. Інші керуючі параметри залишаються встановленими за замовчуванням.
Інші керуючі параметри залишаються встановленими за замовчуванням:
- **birth-threshold**: 15.
- **grass-grow-rate**: 15.
- **grass-energy**: 5.
- **weeds-grow-rate**: 0.
...
Представлено графік останньої симуляції для випадку, коли параметр *number-of-rabbits* дорівнює 500. Графік демонструє, що початкова чисельність кроликів не впливає на середнє значення популяції, оскільки керуючі параметри за замовчуванням забезпечують її стабільність. Для перевірки було також проведено експеримент із початковим значенням *number-of-rabbits* = 20, результати якого виявилися аналогічними.
...
Подані графіки демонструють, що середня чисельність популяції кроликів напряму залежить від кількості енергії, яку вони отримують під час споживання трави. У межах цього експерименту залежність має майже лінійний характер. Цікавим спостереженням є те, що при значенні *grass-energy* = 1 у деяких випадках популяція кроликів припиняла своє існування. Це пояснюється обмеженнями поточної моделі, в якій кролики переміщуються полем випадковим чином, а не активно "шукають" їжу.
...
[Посилання на nlogo файл](model.nlogo)
